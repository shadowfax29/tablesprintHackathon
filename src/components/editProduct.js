...
import { addProduct, updateProduct } from "../actions/productAction"; // Import update action
...
const EditProduct = () => {
    ...
    const { product } = location.state || {}; // Destructure the passed product from location state

    const [categories, setCategories] = useState([]);
    const [subCategories, setSubCategories] = useState([]);
    const [category, setCategory] = useState(product?.category || ""); // Prepopulate with product data
    const [subCategory, setSubCategory] = useState(product?.subcategory || "");
    const [productName, setProductName] = useState(product?.productName || "");
    const [image, setImage] = useState(product?.image || null) // Image upload
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (!category || !subCategory || !productName || (!image && !product?.imageUrl)) {
...
const resetForm=()=>{
    ...
    setProductName("");
        ...
        formData.append("productName", productName);
            ...
            dispatch(updateProduct(formData,resetForm,product._id));
